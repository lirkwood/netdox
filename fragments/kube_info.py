        ...
        for dom in d[context]:
            ...
            with open('../Sources/kube_pods-frag-template.xml', 'r') as stream:
                ...
                for pod in domain['pods']:
                    ...
                    frag.xref['href'] = podlinkbase + pod
                ...
                with open('kube/{0}.psml'.format(filename), 'w') as o:
...
def main():
    ...
    section = 'appman'
    for file in os.scandir('kube'):
        docid = file.name.split(';')[0]
        fragment = file.name.split(';')[1]
        print(docid + ' ' + fragment)
        push.push(os.path.abspath(file), section)
